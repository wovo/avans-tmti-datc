...
void hello_writer(){
    for( int i = 0; i < 10; ++i ){ 
        std::scoped_lock use_cout_exclusively( cout_mutex, counter_mutex );
...
void world_writer(){
    for( int i = 0; i < 10; ++i ){ 
        std::scoped_lock use_counter_exclusively( counter_mutex, cout_mutex );
        //sleep_ns( 1000 );
        std::scoped_lock use_cout_exclusively(  );
